...
export function hearingsRouter(deps: { db: Pool; redis: Redis }) {
  ...
  router.post('/stages/:stageId/hearings', requireAuth, requireRole('admin', 'lawyer'), async (req, res) => {
    ...
    const { rows } = await db.query(
      ...
      [
        req.params.stageId, body.type, body.datetime,
        body.result ?? null, body.appealed ?? null,
        body.newDatetime ?? null, body.adjReason ?? null, body.notes ?? null,
      ],
    );

    await writeAuditLog(db, {
      userId: req.user!.id, action: 'CREATE', entityType: 'hearing',
    ...
    const suggestion = body.type === 'result'
  ...
  router.patch('/hearings/:id', requireAuth, requireRole('admin', 'lawyer'), async (req, res) => {
    const { rows: [hearing] } = await db.query(
    ...
    const map: Record<string, string> = {
      type: 'type', datetime: 'datetime', result: 'result',
      appealed: 'appealed', newDatetime: 'new_datetime',
      adjReason: 'adj_reason', notes: 'notes',
    };
    const sets: string[] = [];
    const vals: unknown[] = [];
    let idx = 1;
  ...
  router.delete('/hearings/:id', requireAuth, requireRole('admin'), async (req, res) => {
    ...
    try {
      ...
      await writeAuditLog(client, {
        userId: req.user!.id, action: 'DELETE', entityType: 'hearing',
