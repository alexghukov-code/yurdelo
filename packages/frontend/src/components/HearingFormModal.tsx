    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          {/* Type */}
          <div>
            <label htmlFor="hf-type" className="block text-sm font-medium text-gray-700 mb-1">
              Тип
            </label>
            <select
              id="hf-type"
          ...
          {/* Datetime */}
          <div>
            <label htmlFor="hf-dt" className="block text-sm font-medium text-gray-700 mb-1">
              Дата и время
            </label>
            <input
              id="hf-dt"
            ...
            />
            {errors.datetime && (
              <p className="text-xs text-red-600 mt-1">{errors.datetime.message}</p>
            )}
          </div>
          ...
          {selectedType === 'result' && (
            <>
              <div>
                <label htmlFor="hf-result" className="block text-sm font-medium text-gray-700 mb-1">
                  Результат
                </label>
                <select
                  id="hf-result"
                  {...register('result', {
                    required: selectedType === 'result' ? 'Результат обязателен.' : false,
                  })}
                ...
                {errors.result && (
                  <p className="text-xs text-red-600 mt-1">{errors.result.message}</p>
                )}
              </div>
          ...
          {selectedType === 'adj' && (
            <>
              <div>
                <label htmlFor="hf-newdt" className="block text-sm font-medium text-gray-700 mb-1">
                  Новая дата
                </label>
                <input
                  id="hf-newdt"
                  ...
                  {...register('newDatetime', {
                    required: selectedType === 'adj' ? 'Новая дата обязательна.' : false,
                  })}
                  className={`w-full rounded-lg border px-3 py-2 text-sm ${errors.newDatetime ? 'border-red-300' : 'border-gray-300'}`}
                />
                {errors.newDatetime && (
                  <p className="text-xs text-red-600 mt-1">{errors.newDatetime.message}</p>
                )}
              </div>
              <div>
                <label htmlFor="hf-reason" className="block text-sm font-medium text-gray-700 mb-1">
                  Причина переноса
                </label>
                <input
                  id="hf-reason"
          ...
          <div>
            <label htmlFor="hf-notes" className="block text-sm font-medium text-gray-700 mb-1">
              Заметки
            </label>
            <textarea
              id="hf-notes"
