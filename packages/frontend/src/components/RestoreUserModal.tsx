...
export function RestoreUserModal({ userId, userName, previousRole, onClose }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: (body: FormValues) => restoreUser(userId, {
      date: body.date,
      role: body.role,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
        ...
        <form onSubmit={handleSubmit((v) => mutation.mutate(v))} className="space-y-3">
          <div>
            <label htmlFor="ru-date" className="block text-sm font-medium text-gray-700 mb-1">Дата</label>
            <input id="ru-date" type="date" {...register('date', { required: 'Обязательно.' })}
              className={`w-full rounded-lg border px-3 py-2 text-sm ${errors.date ? 'border-red-300' : 'border-gray-300'}`} />
            {errors.date && <p className="text-xs text-red-600 mt-1">{errors.date.message}</p>}
          </div>
          <div>
            <label htmlFor="ru-role" className="block text-sm font-medium text-gray-700 mb-1">Роль</label>
            <select id="ru-role" {...register('role')}
          ...
          <div>
            <label htmlFor="ru-comment" className="block text-sm font-medium text-gray-700 mb-1">Комментарий</label>
            <textarea id="ru-comment" rows={2} {...register('comment')}
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
          </div>
