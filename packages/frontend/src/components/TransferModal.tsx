...
export function TransferModal({ caseId, caseName, currentLawyerId, currentLawyerName, onClose }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: (body: FormValues) => createTransfer({
      caseId,
      toId: body.toId,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          <div>
            <label htmlFor="tf-to" className="block text-sm font-medium text-gray-700 mb-1">Кому</label>
            <select id="tf-to"
          ...
          <div>
            <label htmlFor="tf-comment" className="block text-sm font-medium text-gray-700 mb-1">Комментарий</label>
            <textarea id="tf-comment" rows={2} {...register('comment')}
          ...
          {confirmed && selectedName && (
            <div className="flex items-start gap-2 rounded-lg bg-yellow-50 border border-yellow-200 px-3 py-2 text-sm text-yellow-800">
              ...
              <span>
                Дело «{caseName}» будет передано от {currentLawyerName} к {selectedName.lastName} {selectedName.firstName}. Продолжить?
              </span>
            </div>
          )}
